...
var debug = require('debug')('regionsController');
...
RegionsController.buildProjection = function(projections){
...
RegionsController.find = function(req,res,next){
    ...
    if(req.query.search){
        ...
        if(projection){
            ourProjection = RegionsController.buildProjection(projection);
            delete query.select;
        }
        ...
        _query.where = query;
        var totalResult = Regions.count(_query);
        ...
        if(projection){
            ...
            .spread(function(resp,total,totalResult){
                ...
                return [Regions.findAll(_query),total,totalResult];
            })
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
            ...
            q.all([Regions.findAll(_query),total,totalResult])
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
                    ourLastId = null;
...
RegionsController.findOne = function(req,res,next){  
    ...
    _query.where = Regions.where = {_id:id};
    ...
    var question = Regions.findOne(_query);
...
RegionsController.create = function(req,res,next){
    ...
    var question;
    if(data.length){
        question = Regions.bulkCreate(data);
    }else{
        question = Regions.create(data);
    }
...
RegionsController.update = function(req,res,next){
    var query = req.query;
    var _query = {};
    // Clean appId and userId
    if(query && query.appId){
...
RegionsController.delete = function(req,res,next){ 
    var _query = {};
    var query = req.query;
    // Clean appId and userId
    if(query && query.appId){
...
RegionsController.restore = function(req,res,next){
    var id = req.params.id;
    // Find data by ID from trash 
    Trash.findById(id)
    .then(function(resp){
        // Restore to DB
        return Regions.create(resp.data);
    })
    .then(function(resp){
        // Delete from trash
