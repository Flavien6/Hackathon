...
var Inventaires = require('../models').Inventaires;
var Trash = require('../models').Trash;
var q = require('q');
var queue = require('../services/queue');
var debug = require('debug')('inventairesController');
...
InventairesController.buildProjection = function(projections){
...
InventairesController.find = function(req,res,next){
    ...
    if(req.query.search){
        ...
        if(projection){
            ourProjection = InventairesController.buildProjection(projection);
            delete query.select;
        }
        ...
        _query.where = query;
        var totalResult = Inventaires.count(_query);
        var total = Inventaires.count({});
        ...
        if(projection){
            ...
            .spread(function(resp,total,totalResult){
                ...
                return [Inventaires.findAll(_query),total,totalResult];
            })
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
            ...
            q.all([Inventaires.findAll(_query),total,totalResult])
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
                    ourLastId = null;
...
InventairesController.findOne = function(req,res,next){  
    var _query = {};
    ...
    _query.where = Inventaires.where = {_id:id};
    ...
    var question = Inventaires.findOne(_query);
...
InventairesController.create = function(req,res,next){
    ...
    var question;
    if(data.length){
        question = Inventaires.bulkCreate(data);
    }else{
        question = Inventaires.create(data);
    }
...
InventairesController.update = function(req,res,next){
    var query = req.query;
    var _query = {};
    // Clean appId and userId
    if(query && query.appId){
...
InventairesController.delete = function(req,res,next){ 
    var _query = {};
    var query = req.query;
    // Clean appId and userId
    if(query && query.appId){
...
InventairesController.restore = function(req,res,next){
    var id = req.params.id;
    // Find data by ID from trash 
    Trash.findById(id)
    .then(function(resp){
        // Restore to DB
        return Inventaires.create(resp.data);
    })
    .then(function(resp){
        // Delete from trash
