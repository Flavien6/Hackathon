...
var Secteurs = require('../models').Secteurs;
var Trash = require('../models').Trash;
var q = require('q');
var queue = require('../services/queue');
var debug = require('debug')('secteursController');
...
SecteursController.buildProjection = function(projections){
...
SecteursController.find = function(req,res,next){
    ...
    if(req.query.search){
        ...
        if(projection){
            ourProjection = SecteursController.buildProjection(projection);
            delete query.select;
        }
        ...
        _query.where = query;
        var totalResult = Secteurs.count(_query);
        var total = Secteurs.count({});
        ...
        if(projection){
            ...
            .spread(function(resp,total,totalResult){
                ...
                return [Secteurs.findAll(_query),total,totalResult];
            })
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
            ...
            q.all([Secteurs.findAll(_query),total,totalResult])
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
                    ourLastId = null;
...
SecteursController.findOne = function(req,res,next){  
    ...
    _query.where = Secteurs.where = {_id:id};
    ...
    var question = Secteurs.findOne(_query);
...
SecteursController.create = function(req,res,next){
    ...
    var question;
    if(data.length){
        question = Secteurs.bulkCreate(data);
    }else{
        question = Secteurs.create(data);
    }
...
SecteursController.update = function(req,res,next){
    var query = req.query;
    var _query = {};
    // Clean appId and userId
    if(query && query.appId){
...
SecteursController.delete = function(req,res,next){ 
    var _query = {};
    var query = req.query;
    // Clean appId and userId
    if(query && query.appId){
...
SecteursController.restore = function(req,res,next){
    var id = req.params.id;
    // Find data by ID from trash 
    Trash.findById(id)
    .then(function(resp){
        // Restore to DB
        return Secteurs.create(resp.data);
    })
    .then(function(resp){
        // Delete from trash
