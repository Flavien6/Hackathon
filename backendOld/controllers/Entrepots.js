...
var Entrepots = require('../models').Entrepots;
var Trash = require('../models').Trash;
var q = require('q');
var queue = require('../services/queue');
var debug = require('debug')('entrepotsController');
...
EntrepotsController.buildProjection = function(projections){
...
EntrepotsController.find = function(req,res,next){
    ...
    if(req.query.search){
        ...
        if(projection){
            ourProjection = EntrepotsController.buildProjection(projection);
            delete query.select;
        }
        ...
        _query.where = query;
        var totalResult = Entrepots.count(_query);
        var total = Entrepots.count({});
        ...
        if(projection){
            ...
            .spread(function(resp,total,totalResult){
                ...
                return [Entrepots.findAll(_query),total,totalResult];
            })
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
            ...
            q.all([Entrepots.findAll(_query),total,totalResult])
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
                    ourLastId = null;
...
EntrepotsController.findOne = function(req,res,next){  
    ...
    _query.where = Entrepots.where = {_id:id};
    ...
    var question = Entrepots.findOne(_query);
...
EntrepotsController.create = function(req,res,next){
    ...
    var question;
    if(data.length){
        question = Entrepots.bulkCreate(data);
    }else{
        question = Entrepots.create(data);
    }
...
EntrepotsController.update = function(req,res,next){
    var query = req.query;
    var _query = {};
    // Clean appId and userId
    if(query && query.appId){
...
EntrepotsController.delete = function(req,res,next){ 
    var _query = {};
    var query = req.query;
    // Clean appId and userId
    if(query && query.appId){
...
EntrepotsController.restore = function(req,res,next){
    var id = req.params.id;
    // Find data by ID from trash 
    Trash.findById(id)
    .then(function(resp){
        // Restore to DB
        return Entrepots.create(resp.data);
    })
    .then(function(resp){
        // Delete from trash
