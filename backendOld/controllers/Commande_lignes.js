...
var Commande_lignes = require('../models').Commande_lignes;
var Trash = require('../models').Trash;
var q = require('q');
var queue = require('../services/queue');
var debug = require('debug')('commande lignesController');
var _ = require('lodash');

var service = 'Commande_lignes';

var Commande_lignesController = {};

Commande_lignesController.buildProjection = function(projections){
...
Commande_lignesController.find = function(req,res,next){
    var query;
    ...
    if(req.query.search){
        ...
        if(projection){
            ourProjection = Commande_lignesController.buildProjection(projection);
            delete query.select;
        }
        ...
        _query.where = query;
        var totalResult = Commande_lignes.count(_query);
        var total = Commande_lignes.count({});
        ...
        if(projection){
            ...
            .spread(function(resp,total,totalResult){
                ...
                return [Commande_lignes.findAll(_query),total,totalResult];
            })
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
            ...
            q.all([Commande_lignes.findAll(_query),total,totalResult])
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
                    ourLastId = null;
...
Commande_lignesController.findOne = function(req,res,next){  
    var _query = {};
    ...
    _query.where = Commande_lignes.where = {_id:id};
    ...
    var question = Commande_lignes.findOne(_query);
...
Commande_lignesController.create = function(req,res,next){
    var data  = req.body;
    if(data && data.secure){
        delete data.secure;
    }
    ...
    var question;
    if(data.length){
        question = Commande_lignes.bulkCreate(data);
    }else{
        question = Commande_lignes.create(data);
    }
...
Commande_lignesController.update = function(req,res,next){
    var query = req.query;
    var _query = {};
    // Clean appId and userId
    if(query && query.appId){
    ...
    _query.where = query;
    Commande_lignes.update(data, _query)
    .then(function(resp){
        res.ok(resp);
    })
    .catch(function(err){
...
Commande_lignesController.updateOne = function(req,res,next){
    var _query = {};
    var id = req.params.id;
    var data  = req.body;
    if(data && data.secure){
...
Commande_lignesController.delete = function(req,res,next){ 
    var _query = {};
    var query = req.query;
    // Clean appId and userId
    if(query && query.appId){
    ...
    // Find match
    Commande_lignes.findAll(_query)
    .then(function(resp){
        var num = resp.length;
        var last = num - 1;
        for(var n in resp){
...
Commande_lignesController.deleteOne = function(req,res,next){
    var _query = {};
    var id = req.params.id;

    _query.where = {_id:id};
    // Find match
    Commande_lignes.findOne(_query)
    .then(function(resp){
        if(!resp){
...
Commande_lignesController.restore = function(req,res,next){
    var id = req.params.id;
    // Find data by ID from trash 
    Trash.findById(id)
    .then(function(resp){
        // Restore to DB
        return Commande_lignes.create(resp.data);
    })
    .then(function(resp){
        // Delete from trash
...
module.exports = Commande_lignesController;
