...
var Localisations = require('../models').Localisations;
var Trash = require('../models').Trash;
var q = require('q');
var queue = require('../services/queue');
var debug = require('debug')('localisationsController');
...
LocalisationsController.buildProjection = function(projections){
...
LocalisationsController.find = function(req,res,next){
    ...
    if(req.query.search){
        ...
        if(projection){
            ourProjection = LocalisationsController.buildProjection(projection);
            delete query.select;
        }
        ...
        _query.where = query;
        var totalResult = Localisations.count(_query);
        var total = Localisations.count({});
        ...
        if(projection){
            ...
            .spread(function(resp,total,totalResult){
                ...
                return [Localisations.findAll(_query),total,totalResult];
            })
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
            ...
            q.all([Localisations.findAll(_query),total,totalResult])
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
                    ourLastId = null;
...
LocalisationsController.findOne = function(req,res,next){  
    ...
    _query.where = Localisations.where = {_id:id};
    ...
    var question = Localisations.findOne(_query);
...
LocalisationsController.create = function(req,res,next){
    ...
    var question;
    if(data.length){
        question = Localisations.bulkCreate(data);
    }else{
        question = Localisations.create(data);
    }
...
LocalisationsController.update = function(req,res,next){
    var query = req.query;
    var _query = {};
    // Clean appId and userId
    if(query && query.appId){
...
LocalisationsController.delete = function(req,res,next){ 
    var _query = {};
    var query = req.query;
    // Clean appId and userId
    if(query && query.appId){
...
LocalisationsController.restore = function(req,res,next){
    var id = req.params.id;
    // Find data by ID from trash 
    Trash.findById(id)
    .then(function(resp){
        // Restore to DB
        return Localisations.create(resp.data);
    })
    .then(function(resp){
        // Delete from trash
