...
describe('/emploi historiques', function(){
    ...
    describe('Find', function(){
        it('should search for matching documents for a given string', function(done){
            agent
            .get('/emploi historiques?search=femi')
            .set('x-tag',tag)
            .expect(200, done);
        });
        it('should limit the number of returned documents',function(done){
            agent
            .get('/emploi historiques?limit=2')
            .set('x-tag',tag)
            .then(function(resp){
        ...
        it('should sort documents in ascending order', function(done){
            agent
            .get('/emploi historiques?sort=name')
            .set('x-tag',tag)
            .expect(200,done);
        });
        ...
        it('should select just few parameters from the documents', function(done){
            agent
            .get('/emploi historiques?select=name')
            .set('x-tag',tag)
            .expect(200,done);
        });
        // it('should populate data of a reference for multiple data', function(done){
        //     agent
        //     .get('/emploi historiques?populate=toPop')
        //     .set('x-tag',tag)
        //     .then(function(resp){
        ...
        //     agent
        //     .get('/emploi historiques/'+oneId2+'?populate=toPop')
        //     .set('x-tag',tag)
        //     .then(function(resp){
        ...
        it('should load next page for pagination', function(done){
            agent
            .get('/emploi historiques?lastId='+last+'')
            .set('x-tag',tag)
            .expect(200,done);
        });
        it('should filter by date range', function(done){
            agent
            .get('/emploi historiques?from='+from+'&to='+new Date().toISOString())
            .set('x-tag',tag)
            .then(function(resp){
                resp.body.data.length.should.be.above(0);
                done();
        ...
        it('should filter by date range without setting the end date', function(done){
            agent
            .get('/emploi historiques?from='+from)
            .set('x-tag',tag)
            .then(function(resp){
                resp.body.data.length.should.be.above(0);
                done();
...
it('should update documents', function(done){
    agent
    .put('/emploi historiques?name=femi')
    .set('x-tag',tag)
    .send({name: 'Bukola'})
    .then(function(resp){
...
it('should update a document', function(done){
    agent
    .patch('/emploi historiques/'+oneId+'')
    .set('x-tag',tag)
    .send({name: 'Bukola'})
    .then(function(resp){
...
describe('Delete', function(){
    ...
    it('should delete multiple data', function(done){
        agent
        .delete('/emploi historiques?name=femi2')
        .set('x-tag',tag)
        .then(function(resp){
    ...
    it('should delete one data', function(done){
        agent
        .delete('/emploi historiques/'+oneId+'')
        .set('x-tag',tag)
        .then(function(resp){
            resp.body.data.should.be.an('object');
            done();
...
describe('Restore', function(){
    it('should restore a previously deleted data', function(done){
        ...
        setTimeout(function(){
            ...
            .then(function(resp){
                return agent
                .post('/emploi historiques/'+resp._id+'/restore')
                .set('x-tag',tag);
            })
            .then(function(resp){
                resp.body.data.should.be.an('object');
