...
var debug = require('debug')('employesController');
...
EmployesController.buildProjection = function(projections){
...
EmployesController.find = function(req,res,next){
    ...
    if(req.query.search){
        ...
        if(projection){
            ourProjection = EmployesController.buildProjection(projection);
            delete query.select;
        }
        ...
        _query.where = query;
        var totalResult = Employes.count(_query);
        ...
        if(projection){
            ...
            .spread(function(resp,total,totalResult){
                ...
                return [Employes.findAll(_query),total,totalResult];
            })
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
            ...
            q.all([Employes.findAll(_query),total,totalResult])
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
                    ourLastId = null;
...
EmployesController.findOne = function(req,res,next){  
    ...
    _query.where = Employes.where = {_id:id};
    ...
    var question = Employes.findOne(_query);
...
EmployesController.create = function(req,res,next){
    ...
    var question;
    if(data.length){
        question = Employes.bulkCreate(data);
    }else{
        question = Employes.create(data);
    }
...
EmployesController.update = function(req,res,next){
    var query = req.query;
    var _query = {};
    // Clean appId and userId
    if(query && query.appId){
...
EmployesController.delete = function(req,res,next){ 
    var _query = {};
    var query = req.query;
    // Clean appId and userId
    if(query && query.appId){
...
EmployesController.restore = function(req,res,next){
    var id = req.params.id;
    // Find data by ID from trash 
    Trash.findById(id)
    .then(function(resp){
        // Restore to DB
        return Employes.create(resp.data);
    })
    .then(function(resp){
        // Delete from trash
