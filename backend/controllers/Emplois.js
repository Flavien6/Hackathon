...
var Emplois = require('../models').Emplois;
var Trash = require('../models').Trash;
var q = require('q');
var queue = require('../services/queue');
var debug = require('debug')('emploisController');
...
EmploisController.buildProjection = function(projections){
...
EmploisController.find = function(req,res,next){
    ...
    if(req.query.search){
        ...
        if(projection){
            ourProjection = EmploisController.buildProjection(projection);
            delete query.select;
        }
        ...
        _query.where = query;
        var totalResult = Emplois.count(_query);
        var total = Emplois.count({});
        ...
        if(projection){
            ...
            .spread(function(resp,total,totalResult){
                ...
                return [Emplois.findAll(_query),total,totalResult];
            })
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
            ...
            q.all([Emplois.findAll(_query),total,totalResult])
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
                    ourLastId = null;
...
EmploisController.findOne = function(req,res,next){  
    ...
    _query.where = Emplois.where = {_id:id};
    ...
    var question = Emplois.findOne(_query);
...
EmploisController.create = function(req,res,next){
    ...
    var question;
    if(data.length){
        question = Emplois.bulkCreate(data);
    }else{
        question = Emplois.create(data);
    }
...
EmploisController.update = function(req,res,next){
    var query = req.query;
    var _query = {};
    // Clean appId and userId
    if(query && query.appId){
...
EmploisController.delete = function(req,res,next){ 
    var _query = {};
    var query = req.query;
    // Clean appId and userId
    if(query && query.appId){
...
EmploisController.restore = function(req,res,next){
    var id = req.params.id;
    // Find data by ID from trash 
    Trash.findById(id)
    .then(function(resp){
        // Restore to DB
        return Emplois.create(resp.data);
    })
    .then(function(resp){
        // Delete from trash
