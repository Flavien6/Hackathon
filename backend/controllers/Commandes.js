...
var debug = require('debug')('commandesController');
...
CommandesController.buildProjection = function(projections){
...
CommandesController.find = function(req,res,next){
    ...
    if(req.query.search){
        ...
        if(projection){
            ourProjection = CommandesController.buildProjection(projection);
            delete query.select;
        }
        ...
        _query.where = query;
        var totalResult = Commandes.count(_query);
        ...
        if(projection){
            ...
            .spread(function(resp,total,totalResult){
                ...
                return [Commandes.findAll(_query),total,totalResult];
            })
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
            ...
            q.all([Commandes.findAll(_query),total,totalResult])
            .spread(function(resp,total,totalResult){
                var ourLastId;
                if(resp.length === 0){
                    ourLastId = null;
...
CommandesController.findOne = function(req,res,next){  
    ...
    _query.where = Commandes.where = {_id:id};
    ...
    var question = Commandes.findOne(_query);
...
CommandesController.create = function(req,res,next){
    ...
    var question;
    if(data.length){
        question = Commandes.bulkCreate(data);
    }else{
        question = Commandes.create(data);
    }
...
CommandesController.update = function(req,res,next){
    var query = req.query;
    var _query = {};
    // Clean appId and userId
    if(query && query.appId){
...
CommandesController.delete = function(req,res,next){ 
    var _query = {};
    var query = req.query;
    // Clean appId and userId
    if(query && query.appId){
...
CommandesController.restore = function(req,res,next){
    var id = req.params.id;
    // Find data by ID from trash 
    Trash.findById(id)
    .then(function(resp){
        // Restore to DB
        return Commandes.create(resp.data);
    })
    .then(function(resp){
        // Delete from trash
